...
"""
  dirac-rss-db
    ...
    Usage:
        dirac-rss-db [option] <query> <element> <tableType>
    ...
    Options:
        -N --name=               ElementName (it admits a comma-separated list of element names); None by default
        -X --statusType=         A valid StatusType argument (it admits a comma-separated list of statusTypes
                              e.g. ReadAccess, WriteAccess, RemoveAccess ); None by default
        -S --status=             A valid Status argument ( Active, Probing, Degraded, Banned, Unknown, Error );
                              None by default
        -Y --elementType=        ElementType narrows the search (string, list); None by default
        -R --reason=             Decision that triggered the assigned status
        -L --lastCheckTime=      Time-stamp setting last time the status & status were checked
        -O --tokenOwner=         Owner of the token ( to specify only with select/delete queries )
...
def registerSwitches():
  ...
  switches = ( 
    ( "E:", 'element=', 'Element family to be Synchronized ( Site, Resource, Node )' ),
    ( "T:", 'tableType=', 'A valid table type (Status, Log, History)' ),
    ( "N:", 'name=', 'ElementName; None if default' ),
    ( "X", 'statusType=', 'A valid StatusType argument (it admits a comma-separated list of statusTypes); None if default' ),
    ( "S:", 'status=', 'A valid Status argument ( Active, Probing, Degraded, Banned, Unknown, Error ); None if default' ),
    ( "Y:", 'elementType=', 'ElementType narrows the search; None if default' ),
    ( "R:", 'reason=', 'Decision that triggered the assigned status' ),
    ( "L:", 'lastCheckTime=', 'Time-stamp setting last time the status & status were checked' ),
    ( "O:", 'tokenOwner=', 'Owner of the token ( to specify only with select/delete queries )' ),
             )
  ...
  for switch in switches:
    Script.registerSwitch( switch[ 0 ], switch[ 1 ], switch[ 2 ] )
...
def parseSwitches():
  ...
  if 'N' in switches:
    switches['name'] = switches['N']
  if 'X' in switches:
    switches['statusType'] = switches['X']
  if 'S' in switches:
    switches['status'] = switches['S']
  if 'Y' in switches:
    switches['elementType'] = switches['Y']
  if 'R' in switches:
    switches['reason'] = switches['R']
  if 'L' in switches:
    switches['lastCheckTime'] = switches['L']
  if 'O' in switches:
    switches['tokenOwner'] = switches['O']
