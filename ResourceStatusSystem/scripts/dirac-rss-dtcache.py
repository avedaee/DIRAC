...
"""
  dirac-rss-dtcache
    ...
    Usage:
        dirac-rss-dtcache [option] <query>

    Queries:
        [select|add|delete]

    Options:
        -I --downtimeID=         The ID of the downtime
        -E --element=            Element (Site, Service) affected by the downtime
        -N --name=               Name of the element
        -B --startDate=          Starting date of the downtime
        -F --endDate=            Ending date of the downtime
        -S --severity=           Severity of the downtime (Warning, Outage)
        -D --description=        Description of the downtime
        -L --link=               URL of the downtime announcement
        -O --ongoing             To force "select" to return the ongoing downtimes
...
def registerSwitches():
  ...
  switches = ( 
    ( "I:", 'downtimeID=', 'ID of the downtime' ),
    ( "E:", 'element=', 'Element (Site, Service) affected by the downtime' ),
    ( "N:", 'name=', 'Name of the element' ),
    ( "B:", 'startDate=', 'Starting date of the downtime' ),
    ( "F:", 'endDate=', 'Ending date of the downtime' ),
...
def parseSwitches():
  ...
  if 'I' in switches:
    switches['downtimeID'] = switches['I']
  if 'E' in switches:
    switches['element'] = switches['E']
  if 'N' in switches:
    switches['name'] = switches['N']
  if 'B' in switches:
    switches['startDate'] = switches['B']
  if 'F' in switches:
    switches['endDate'] = switches['F']
  if 'S' in switches:
    switches['severity'] = switches['S']
  if 'D' in switches:
    switches['description'] = switches['D']
  if 'L' in switches:
    switches['link'] = switches['L']
  #if 'O' in switches:
  ...
  if query in ( 'add', 'delete' ):
    if "O" in switches or 'ongoing' in switches:
...
def select( switchDict ):
  ...
  if 'ongoing' in switchDict or 'O' in switchDict:
